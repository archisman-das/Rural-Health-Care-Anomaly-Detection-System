"""Variational autoencoder detector for tabular anomaly scoring."""
...
class VariationalAutoencoder(BaseEstimator, OutlierMixin):
    """A compact variational autoencoder with reconstruction-threshold scoring."""
    ...
    def _forward(self, X: np.ndarray, *, training: bool) -> tuple[np.ndarray, _VAEForwardCache]:
        ...
        mu = encoder_activation @ self.weights_[1] + self.biases_[1]
        logvar = encoder_activation @ self.weights_[2] + self.biases_[2]
        logvar = np.clip(logvar, -10.0, 10.0)
        std = np.exp(0.5 * logvar)

        if training:
            eps = np.random.default_rng(self.random_state + self._adam_step_ + 11).standard_normal(mu.shape)
        else:
            eps = np.zeros_like(mu)
        latent = mu + std * eps

        decoder_linear = latent @ self.weights_[3] + self.biases_[3]
        ...
        reconstruction = decoder_activation @ self.weights_[4] + self.biases_[4]
        cache = _VAEForwardCache(
    ...
    def _kl_divergence(self, cache: _VAEForwardCache) -> np.ndarray:
        return 0.5 * np.sum(np.exp(cache.logvar) + cache.mu**2 - 1.0 - cache.logvar, axis=1)

    def _loss(self, X: np.ndarray, recon: np.ndarray, cache: _VAEForwardCache) -> float:
        reconstruction = np.mean((recon - X) ** 2)
        kl = np.mean(self._kl_divergence(cache))
        return float(reconstruction + self.beta * kl)
    ...
    def _backward(
        ...
        delta_out = (2.0 / batch_size) * (reconstruction - X)
        grads_w[4] = cache.decoder_activation.T @ delta_out + self.l2 * self.weights_[4]
        grads_b[4] = delta_out.sum(axis=0)

        delta_decoder = delta_out @ self.weights_[4].T
        ...
        grads_w[3] = cache.latent.T @ delta_decoder + self.l2 * self.weights_[3]
        grads_b[3] = delta_decoder.sum(axis=0)
        delta_latent = delta_decoder @ self.weights_[3].T

        kl_mu_grad = (self.beta / batch_size) * cache.mu
        kl_logvar_grad = (self.beta / (2.0 * batch_size)) * (np.exp(cache.logvar) - 1.0)

        delta_mu = delta_latent + kl_mu_grad
        delta_logvar = delta_latent * (0.5 * cache.std * cache.eps) + kl_logvar_grad

        grads_w[1] = cache.encoder_activation.T @ delta_mu + self.l2 * self.weights_[1]
        grads_b[1] = delta_mu.sum(axis=0)
        grads_w[2] = cache.encoder_activation.T @ delta_logvar + self.l2 * self.weights_[2]
        grads_b[2] = delta_logvar.sum(axis=0)

        delta_encoder = delta_mu @ self.weights_[1].T + delta_logvar @ self.weights_[2].T
    ...
    def fit(self, X, y=None):
        ...
        if X.shape[0] < 2:
            raise ValueError("Variational autoencoder requires at least two samples.")
        ...
        for epoch in range(self.max_epochs):
            ...
            train_loss = float(np.mean(train_recon))
            val_loss = float(np.mean(val_recon))
